...
def test3(tagger):
    ...
    tags: List[Tag] = result["tags"]
    assert tags[0].is_proclisi is True
    assert tags[1].is_proclisi is True
    assert tags[2].is_proclisi is True
    assert tags[3].is_proclisi is True


def test4(tagger):
    result = tagger.tag("A me servono delle penne")
    tags: List[Tag] = result["tags"]
    assert tags[0].is_proclisi is None
    assert tags[1].is_proclisi is None
