...
def process_next_block(
    ...
    if tmp.any_occurrences_in_block(["ci", "c'"]):  # Delle soluzioni ci vogliono
        if tmp.is_lemma_in_block("volere") or all(
            ...
            t.pos = "DET:part"
            notes.append(
                f"{t.occorrenza.upper()} is the partitive article of the topicalised subject"
            )
            blocks_done.add(t.block)
            return
    else:
        if t.block == 1 or next_block[0].any_lemmas_in_block(reggono_di):
            g = [x.numero for x in t.get_next_block_tags() if x.is_inflected_verb()]
            h = [x.persona for x in t.get_next_block_tags() if x.is_inflected_verb()]
            for p in tags_next_2_blocks:
                l = [x for x in t.get_next_block_tags() if x.is_inflected_verb()]
                if p.lemma in agg_trigger_di:
                    if (
                        "p" in g and "3rd" in h
                    ):  # Delle conseguenze/Dei professori sono consci ['noi' iniziale crea problemi]
                        notes.append(
                            f"Ambiguous phrase: {t.occorrenza.upper()} is either a partitive article or\
                        ...
                        notes.append(
                            f"{t.occorrenza.upper()} is \
                            a topicalized genitive licensed by {p.lemma.upper()}"
                        )
                        blocks_done.add(t.block)
                        return
                # e.g. Dei risultati sono soddisfatte/Delle amiche sono soddisfatte
                if p.lemma in agg_trigger_di_pp:
                    notes.append(
                        f"Ambiguous phrase: {t.occorrenza.upper()} is either a a partitive article or\
                        a topicalized genitive licensed by {p.lemma.upper()}"
                    )
                    t.pos = "DET:part|PRE:det"
                    blocks_done.add(t.block)
            ...
            for p in t.get_next_block_tags():  # Dei risultati noi siamo soddisfatti
                if p.occorrenza in agg_trigger_di_pp:
                    notes.append(
                        f"Ambiguous phrase: {t.occorrenza.upper()} is either a a partitive article or\
                        a topicalized genitive licensed by {p.occorrenza.upper()}"
                    )
                    t.pos = "DET:part|PRE:det"
                    blocks_done.add(t.block)
                    return
            for m in t.get_next_block_tags():
                if m.lemma in verb_trigger_di | verb_trigger_di_pron:
                    if t.match_pn(m):
                        if any(
                            x.lemma in ["di", "del"] for x in tags_next_2_blocks
                        ):  # Dei tipi parlano di noi/Delle ragazze si fidano di noi
                            notes.append(
                                f"{t.occorrenza.upper()} (index {t.index}) is a partitive article"
                            )
                            t.pos = "DET:part"
                            blocks_done.add(t.block)
                            return
                        ...
                        notes.append(
                            f"{t.occorrenza.upper()} introduces a topicalized genitive licensed by {m.lemma.upper()}"
                        )
                        blocks_done.add(t.block)
                        return
                else:
                    if m.is_inflected_verb():
                        ...
                        if t.match_pn(m):
                            if (
                                ...
                                and m_block[-1].lemma
                                not in verb_trigger_di | verb_trigger_di_pron
                            ):
                        ...
                        if m.is_passive() and t.match_pn(
                            ...
                            if t.match_pn(m):  # Delle ragazze hanno visto
                                ...
                                if m.lemma not in [
                                    ...
                                    notes.append(
                                        f"{t.occorrenza.upper()} is a partitive article (topicalised)"
                                    )
                                    t.pos = "DET:part"
                                    blocks_done.add(t.block)
                                    return
