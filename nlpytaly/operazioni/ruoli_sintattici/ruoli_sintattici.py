...
from .exclude_meteo_verbs import exclude_meteo_verbs
from .ricerca_dx import candidato_piu_vicino_dx_verbs
from .ricerca_sx import candidato_piu_vicino_sx
...
def ruoli_sintattici(
    ...
    exclude_meteo_verbs(tags, indici_verbi_flessi)
    if exclusions is None:
        exclusions = []  # Lui fa >il meccanico<

    for step in [1, 2]:
        # sp = soggetto presunto
        results = candidato_piu_vicino_sx(
            indici_verbi_flessi,
            indici_proclisi,
            indici_candidati,
            tags,
        ...
        # op = oggetto presunto
        op = candidato_piu_vicino_dx_verbs(
            ...
            if (
                ...
                if v_tag.match_pn(o_tag):
                    if (
                        ...
                        or (
                            v_tags[0].is_active()
                            and any(v.lemma in verbi_classe_1 for v in v_tags)
                        )
                    ):
