    ...
    if infi_esiste:
        det_esiste = any(
            [
                x
                for x in t.get_same_block_tags()
                if x.pos in determiners and x.index < infi_index[0]
            ]
        ...
        if det_esiste:  # Per me il continuo scorrere delle immagini è piacevole
            ...
            notes.append(
                f"{t.occurrence.upper()} (index {t.index}) introduces a partitive"
            )
            t.pos = "DET:part"
            blocks_done.add(t.block)
            return
    ...
    for x in t_block:
        if (
            ...
            if (
                ...
                blocks_done.add(t.block)
                return
        if tags[t.index - 1].occurrence == "prima":
            notes.append(
                f"{t.occurrence.upper()} (index {t.index}) introduces a genitive phrase"
            )
            blocks_done.add(t.block)
        ...
        if tags[t.index - 1].pos == "ADJ" and tags[t.index - 2].lemma in [
            ...
            if tags[t.index - 3].lemma == "il":  # è la più alta delle sorelle
                ...
                notes.append(
                    f"{t.occurrence.upper()} introduces the second member of a comparative"
                )
            blocks_done.add(t.block)
            return
        if x.lemma in adj_trigger_di:  # Lea è conscia delle difficoltà
            notes.append(
                f"{t.occurrence.upper()} introduces a genitive licensed by the adjective {x.lemma.upper()}"
            )
            blocks_done.add(t.block)
            return
